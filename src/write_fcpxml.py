...
def write_fcpxml_from_canonical(canon: Dict[str, Any], out_path: str) -> None:
    ...
    timeline_name = timeline.get("name", "Untitled Timeline")
...
def create_assets(events: List[Dict[str, Any]], resources: ET.Element, fps: float) -> tuple[List[ET.Element], Dict[str, str]]:
...
def create_spine_events(spine: ET.Element, events: List[Dict[str, Any]], asset_map: Dict[str, str], fps: float) -> None:
...
def add_effect_to_clip(clip_elem: ET.Element, effect: Dict[str, Any], fps: float) -> None:
    ...
    if any(keyword in effect_name.lower() for keyword in ["dissolve", "cross", "fade"]):
        ...
        transition = ET.SubElement(clip_elem, "transition", name="Cross Dissolve")
        ...
        for param_name, value in parameters.items():
            if param_name.lower() in ["scale", "scaleX", "scaleY"] and isinstance(value, (int, float)):
